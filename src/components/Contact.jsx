...
const Contact = () => {
  ...
  const handleSubmit =(e)=>{
    e.preventDefault();
    setLoading(true);
    emailjs.send('service_zlo4hoj','template_a6alijs',
      {
        from_name:form.name,
        to_name:'Ashutosh',
        from_email: form.email,
        to_email: 'mr.proashutosh@gmail.com',
    ...
    <div className="xl:mt-12 xl:flex-row flex-col-reverse flex gap-10 overflow-hidden">
      ...
      >
        ...
        <form action="" ref={formRef} onSubmit={handleSubmit} className="mt-12 flex flex-col gap-8">
          ...
          <button type="submit" className="py-3 px-8 bg-slate-600 outline-none w-fit text-white font-bold shadow-md shadow-primary rounded-2xl">{loading ?'sending...':'Send'}</button>
        </form>
      </motion.div>
